...
arr_np = np.array([1, 3, 67, 2, 0 ,23, 3,0,1,0,5,3,6,8,0,56,9,0,56,0])
...
df["peso"] = df["peso"].fillna(np.mean([69.66, 79.77]))
df
...
salida = df[["colesterol", "peso"]]

df = pd.read_csv( 'https://raw.githubusercontent.com/asalber/manual-python/master/datos/colesterol.csv')

df

df = pd.concat([df, pd.DataFrame([{'nombre': 'Carlos Rivas', 'edad': 28, 'sexo': 'H', 'peso': 89.0, 'altura': 1.78, 'colesterol': 245.0}])], ignore_index=True)
